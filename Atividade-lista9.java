public class Main {
    public static void main(String[] args) {
        Pessoa p = new Pessoa("Gian Lucca", 18, "000.000.000-00", "gian@example.com");
        p.andar();
        p.falar();

        Animal a = new Animal("Cão", 3, "Marrom", 12.5);
        a.emitirSom();
        a.correr();

        Carro car = new Carro("Toyota", "Corolla", "Prata", "ABC-1234");
        car.ligar();
        car.acelerar();

        Celular cel = new Celular("Samsung", "S21", "6.2\"", "Preto");
        cel.ligar();
        cel.tirarFoto();

        Computador pc = new Computador("Intel i5", 8, 512, "Windows 10");
        pc.ligar();
        pc.rodarPrograma("Calculadora");

        Livro livro = new Livro("O Pequeno Príncipe", "Antoine de Saint-Exupéry", "Ficção", 96);
        livro.abrir();
        livro.lerPagina(5);

        Escola escola = new Escola("Escola", "Rua A, 123", 10, "Dra. Silva");
        escola.ensinar();
        escola.realizarProvas();

        Televisao tv = new Televisao("LG", "55\"", "4K", "Preto");
        tv.ligar();
        tv.trocarCanal(7);

        Relogio rel = new Relogio("Casio", "Digital", "Preto", "Borracha");
        rel.mostrarHora();
        rel.despertar();

        Bicicleta bike = new Bicicleta("Caloi", "Vermelha", 18, "MTB");
        bike.pedalar();
        bike.frear();
    }
